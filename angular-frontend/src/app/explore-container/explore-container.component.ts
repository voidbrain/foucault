...
export class ExploreContainerComponent implements AfterViewInit, OnDestroy {
  ...
  updateTHREERobotWheelMovementValue!: {
  ...
  onSensorToggled(isEnabled: boolean) {
    ...
    this.sendEnableSensorCommand(
      this.topics.output['enableSensorAdjustements'],
      isEnabled
    );
  }
  ...
  async getConfig() {
    ...
    this.heightLevelIndex = this.config?.heightLevels.findIndex(
      (level: any) => level === this.config.heightLevel
    );
  ...
  updateKp(Kp: number) {
    ...
    this.socketService.emit('message', {
      topic: this.topics.output['setKp'],
      value: Kp.toString(),
      souce: 'Angular FE',
    });
  }
  ...
  updateKi(Ki: number) {
    this.config.Ki = Ki;
    this.socketService.emit('message', {
      topic: this.topics.output['setKi'],
      value: Ki.toString(),
      souce: 'Angular FE',
    });
  }
  ...
  updateKd(Kd: number) {
    this.config.Kd = Kd;
    this.socketService.emit('message', {
      topic: this.topics.output['setKd'],
      value: Kd.toString(),
      souce: 'Angular FE',
    });
  }
  ...
  updateIncrementDegree(incrementDegree: number) {
    ...
    this.socketService.emit('message', {
      topic: this.topics.output['setincrementDegree'],
      value: incrementDegree.toString(),
      source: 'Angular FE',
    });
  }
  ...
  onServoLeftChanged(value: number) {
    const obj = {
      topic: this.topics.output['setServoLeft'],
  ...
  onServoRightChanged(value: number) {
    const obj = {
      topic: this.topics.output['setServoRight'],
      value: value.toString(),
      source: 'Angular FE',
    };
    console.log(obj);
  ...
  onMotorLeftChanged(value: number) {
    this.socketService.emit('message', {
      topic: this.topics.output['setMotorLeft'],
      value: value.toString(),
      source: 'Angular FE',
    });
  }
  ...
  onMotorRightChanged(value: number) {
    this.socketService.emit('message', {
      topic: this.topics.output['setMotorRight'],
      value: value.toString(),
      source: 'Angular FE',
    });
  }
  ...
  setupSocket() {
    ...
    this.socketService
      .onEvent<MqttMessage>('mqtt-message')
      .subscribe((message) => {
  ...
  sendSetHeightCommand(height: string) {
    this.socketService.emit('message', {
      topic: this.topics.output['setHeight'],
      value: height,
      source: 'Angular FE',
    });
  }
  ...
  sendStartCommand() {
    this.socketService.emit('message', {
      topic: this.topics.output['start'],
      source: 'Angular FE',
    });
  }
