...
function wakeUpMPU6050() {
  wire.writeBytes(PWR_MGMT_1, [0x00], (err) => {
    if (err) {
      sendMQTTMessage('pid/console', { message: "Error waking up MPU6050", error: err });
    } else {
      sendMQTTMessage('pid/console', { message: "MPU6050 awake" });
    }
  });
}
...
async function getTiltAngles() {
  try {
    ...
    sendMQTTMessage('pid/console', { message: `Error getting tilt angles: ${error}` });
...
function updateMotors(leftOutput, rightOutput) {
  ...
  sendMQTTMessage('pid/motors/left', { message: `Motor PID Output - Left: ${clampedLeftOutput}`, value: clampedLeftOutput });
  sendMQTTMessage('pid/motors/right', { message: `Motor PID Output - Right: ${clampedRightOutput}`, value: clampedRightOutput });
...
function adjustServos(xAngle) {
  ...
  sendMQTTMessage('pid/servos/left', { message: `Servo Adjustment - Left Height: ${leftHeight}`, value: clampedLeftPulseWidth });
  sendMQTTMessage('pid/servos/right', { message: `Servo Adjustment - Right Height: ${rightHeight}`, value: clampedRightPulseWidth });
